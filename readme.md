...
5.在打开的浏览器里登录x账号
6.(可选)如果要统计xhunt数据的话安装xhunt建议安装xhunt
7.打开cmd运行脚本python extract_users.py
8.cmd里输入你的关注页面链接 https://x.com/ThanksuTrump/following 把ThanksuTrump换成你的账号 不要直接回车用默认页面
9.完毕后手动按ctrl+c停止 如果过程中网页卡住自己动动让他继续滚动 脚本会生成一个csv文件
10.使用excel进行筛选 将xhunt评分一栏升序排列 是否关注我一栏 过滤选择否
11.排除掉前面xhunt评分高的大v你就知道哪些人你关注了但他没回关你 接下来选择要一个个联系让回关还是直接取关
