...
def 初始化浏览器():
    ...
    try:
        ...
        print(f'{Fore.YELLOW}请确保浏览器已通过调试模式启动 例如: C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe" --remote-debugging-port=9222 --user-data-dir="d:\\chromedata"')
        sys.exit(1)
...
def 获取用户输入():
    ...
    目标链接 = input(f"请输入主页/关注列表链接(例如:https://x.com/ThanksuTrump/following) (回车默认: 当前页面): ").strip()
