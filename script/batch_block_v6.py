...
def 执行主逻辑():
    """程序入口函数"""
    print("==================================================")
    print("       X 批量斩杀/取关工具 (全中文版)             ")
    print("==================================================")

    # 选择运行模式
    print("[模式选择]")
    print("  1 - 斩杀模式（检测对方是否关注我，未关注则屏蔽）")
    print("  2 - 仅取关模式（仅取消我对列表用户的关注）")
    模式输入 = input("[设置] 请选择运行模式 (1/2，默认 1): ").strip()
    运行模式 = 模式输入 if 模式输入 in ('1', '2') else '1'
    模式名称 = '斩杀模式' if 运行模式 == '1' else '仅取关模式'
    print(f"[信息] 已选择：{模式名称}")
    
    # 获取用户设置的随机延迟范围
    输入最小 = input("[设置] 请输入随机等待的最小秒数 (默认 3 如数量大建议300过风控): ")
    try:
    ...
    输入最大 = input("[设置] 请输入随机等待的最大秒数 (默认 10 如数量大建议600过风控): ")
    try:
    ...
    try:
        with sync_playwright() as 播放器:
            ...
            for 账户名 in 待处理用户:
                ...
                # --- 账号异常检测（两种模式通用） ---
                网页标题 = 页面对象.title()
                if any(关键词 in 网页标题 for 关键词 in ["Account suspended", "Page not found", "此账号已被冻结", "该账号已被停用"]):
                ...
                if 关注我标志.count() > 0:
                    ...
                    if any(关键词 in 文本内容 for 关键词 in ["Follows you", "关注了你"]):
                        print(f"【验证结果】对方 [已关注] 我 (检测到关注标志)")
                        对方关注我吗 = True
                    else:
                        print(f"【验证结果】对方 [未关注] 我 (标志位文本不匹配: {文本内容})")
                else:
                    print(f"【验证结果】对方 [未关注] 我 (未发现关注标志)")

                # ==================== 模式分支 ====================
                if 运行模式 == '2':
                    # ---------- 仅取关模式 ----------
                    if 对方关注我吗:
                        print(f"[跳过] 对方已关注我，不执行取关操作。")
                    else:
                        print(f"[扫描] 正在检测我对 @{账户名} 的关注状态...")
                        try:
                            # 优先通过按钮文本检测"Following"/"已关注"来判断普通账号
                            普通关注按钮 = 页面对象.locator('button', has_text=re.compile(r'^Following$|^已关注$'))
                            
                            if 普通关注按钮.count() > 0 and 普通关注按钮.first.is_visible():
                                # ========== 普通账号取关流程 ==========
                                print(f"[状态] 我目前：已关注该用户（普通模式），准备执行取关...")
                                
                                # 步骤1：点击 "Following" 按钮触发取关弹窗
                                print(f'[步骤1] 正在点击"已关注"按钮...')
                                拟人驱动.点击元素(普通关注按钮.first)
                                time.sleep(random.uniform(1, 2))
                                
                                # 步骤2：在确认弹窗中点击确认取关
                                确认取关按钮 = 页面对象.locator('[data-testid="confirmationSheetConfirm"]')
                                if 确认取关按钮.count() > 0 and 确认取关按钮.first.is_visible():
                                    print(f'[步骤2] 正在确认取关...')
                                    拟人驱动.点击元素(确认取关按钮.first)
                                    print(f"【执行结果】成功取关用户 @{账户名}（普通模式）")
                                else:
                                    print(f'[错误] 无法定位"确认取关"弹窗按钮')
                            else:
                                # ========== 订阅账号取关流程 ==========
                                # 已开通订阅的账号没有 "Following" 文本按钮
                                # 而是显示带勾号人物图标的订阅者按钮
                                # 通过 SVG 中人物+勾号的特征路径定位该按钮
                                订阅按钮 = 页面对象.locator('[role="button"]').filter(
                                    has=页面对象.locator('path[d*="M14 6c0 2.21"]')
                                )
                                
                                if 订阅按钮.count() > 0 and 订阅按钮.first.is_visible():
                                    print(f"[状态] 我目前：已关注该用户（订阅模式），准备执行取关...")
                                    
                                    # 步骤1：点击订阅者图标按钮，触发下拉菜单
                                    print(f'[步骤1] 正在点击订阅者图标按钮...')
                                    拟人驱动.点击元素(订阅按钮.first)
                                    time.sleep(random.uniform(1, 2))
                                    
                                    # 步骤2：在下拉菜单中定位并点击 "Unfollow" 选项
                                    取关菜单项 = 页面对象.locator(
                                        '[data-testid="Dropdown"] [role="menuitem"]'
                                    ).filter(has_text=re.compile(r'Unfollow|取消关注'))
                                    
                                    if 取关菜单项.count() > 0 and 取关菜单项.first.is_visible():
                                        print(f'[步骤2] 正在点击下拉菜单中的"取关"选项...')
                                        拟人驱动.点击元素(取关菜单项.first)
                                        print(f"【执行结果】成功取关用户 @{账户名}（订阅模式）")
                                    else:
                                        print(f'[错误] 无法在下拉菜单中定位"取关"选项')
                                else:
                                    # 再检查是否有 Follow 按钮（说明本来就没关注）
                                    关注按钮 = 页面对象.locator('button', has_text=re.compile(r'^Follow$|^关注$'))
                                    if 关注按钮.count() > 0 and 关注按钮.first.is_visible():
                                        print(f"[跳过] 我未关注 @{账户名}，无需取关。")
                                    else:
                                        print(f"[疑问] 无法识别关注状态 (按钮未加载或页面结构变化)")
                        except Exception as 操作错误:
                            print(f"[失败] 取关操作流程中断: {操作错误}")

                else:
                    # ---------- 斩杀模式 ----------
                    # 页面关注按钮状态检测
                    try:
                        已关注按钮 = 页面对象.locator('button', has_text=re.compile(r"^Following$|^已关注$"))
                        关注按钮 = 页面对象.locator('button', has_text=re.compile(r"^Follow$|^关注$"))
